...
void Foam::ensightSurfaceReader::skip(const label n, IFstream& is) const
{
    ...
    if (i != n)
    {
        WarningIn
        (
            "void ensightSurfaceReader::skip(const label, IFstream&) const"
        )
...
) const
{
    ...
    if (expected != actual)
    {
        FatalIOErrorIn
        (
            "void Foam::ensightSurfaceWriter::debugSection"
            "("
                "const word&, "
                "IFstream&"
            ") const",
...
void Foam::ensightSurfaceReader::readCase(IFstream& is)
{
    if (debug)
    {
        Info<< "ensightSurfaceReader::readCase(IFstream&)" << endl;
    }
    ...
    if (!is.good())
    {
        FatalErrorIn
        (
            "Foam::ensightSurfaceReader::ensightSurfaceReader((const fileName&)"
...
const Foam::meshedSurface& Foam::ensightSurfaceReader::geometry()
{
    if (debug)
    {
        Info<< "ensightSurfaceReader::geometry()" << endl;
    }
