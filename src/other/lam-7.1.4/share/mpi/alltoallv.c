 ...
 * $HEADER$
 *
 * $Id: alltoallv.c,v 6.14 2004/03/02 03:43:28 jsquyres Exp $
 *
...
{
        int err;
	lam_ssi_coll_alltoallv_fn_t func;

	lam_initerr();
	lam_setfunc(BLKMPIALLTOALLV);
/*
 ...
 */
	...
	func = comm->c_ssi_coll.lsca_alltoallv;
	...
	if ((err = func(sbuf, scounts, sdisps, sdtype, rbuf, rcounts,
			rdisps, rdtype, comm)) != MPI_SUCCESS)
	  return(lam_errfunc(comm, BLKMPIALLTOALLV,
