...
dnl
dnl $Id: lam_rpi_sysv_setup.m4,v 1.5 2004/01/28 02:30:02 jsquyres Exp $
dnl

AC_DEFUN([LAM_RPI_SYSV_SETUP],[
#
# Arguments: none
#
# Dependencies: None
...
if test "$LAM_HAVE_SYSV_SEMAPHORES" = "0" -o "$LAM_HAVE_SYSV_SHMEM" = "0"; then
    AC_MSG_WARN([*** sysv RPI cannot be build on a machine without])
    AC_MSG_WARN([*** SysV semaphores or shared memory.])
    AC_MSG_ERROR([*** Aborting building sysv RPI.])
fi
...
AC_ARG_WITH(shmshort, 
    AC_HELP_STRING([--with-shm-short=BYTES],
		   [obsolete synonym for --with-rpi-sysv-short]),
    [SHMSHORT=$withval])
AC_ARG_WITH(rpi-sysv-short, 
    AC_HELP_STRING([--with-rpi-sysv-short=BYTES],
...
AC_ARG_WITH(shm-poolsize, 
    AC_HELP_STRING([--with-shm-poolsize=BYTES],
		   [obsolete synonym for --with-rpi-sysv-poolsize]),
    [SHMPOOL=$withval])
AC_ARG_WITH(rpi-sysv-poolsize, 
    AC_HELP_STRING([--with-rpi-sysv-poolsize=BYTES],
...
AC_ARG_WITH(shm-maxalloc, 
    AC_HELP_STRING([--with-shm-maxalloc=BYTES],
		   [obsolete synonym for --with-rpi-sysv-maxalloc]),
    [SHMALLOC=$withval])
AC_ARG_WITH(rpi-sysv-maxalloc, 
    AC_HELP_STRING([--with-rpi-sysv-maxalloc=BYTES],
...
AC_ARG_WITH(pthread_lock, 
    AC_HELP_STRING([--with-pthread-lock],
		   [obsolete synonym for --with-rpi-sysv-pthread-lock]),
    [WANT_PTHREAD_LOCK_STRING=$withval])
AC_ARG_WITH(rpi-sysv-pthread_lock, 
    AC_HELP_STRING([--with-rpi-sysv-pthread-lock],
