 ...
 * $HEADER$
 *
 * $Id: ssi_rpi_ib_reqlist.c,v 1.1 2003/11/11 18:36:55 vsahay Exp $
 *
 ...
 */
int
lam_ssi_rpi_ib_reqlist_init(struct lam_ssi_rpi_ib_reqlist_t *rl)
{
  if (rl == NULL)
    return LAMERROR;
 ...
 */
int
lam_ssi_rpi_ib_reqlist_append(struct lam_ssi_rpi_ib_reqlist_t *rl, 
			      MPI_Request req)
{
  struct lam_ssi_rpi_ib_reqlist_item_t *rli;

  rli = malloc(sizeof(struct lam_ssi_rpi_ib_reqlist_item_t));
  if (rli == NULL)
    return LAMERROR;
  ...
  if (rl->size == 0)
    rl->top = rli;
  ++(rl->size);
  lam_debug_cond((lam_ssi_rpi_ib_did, "reqlist_append: rl %p, req %p, "
...
lam_ssi_rpi_ib_reqlist_match(struct lam_ssi_rpi_ib_reqlist_t *rl, 
                             struct lam_ssi_rpi_ib_envl *env)
{
  struct lam_ssi_rpi_ib_reqlist_item_t *rli = NULL;
  ...
  for (rli = rl->top; rli != NULL; rli = rli->next) {
    lam_debug_cond((lam_ssi_rpi_ib_did, 
                    ...
                    rli->req));
    if (lam_ssi_rpi_envl_cmp(&(env->ge_env),
                             &(rli->req->rq_rpi->cq_envbuf->ge_env)) == 0 &&
 ...
 */
int
lam_ssi_rpi_ib_reqlist_rm_item(struct lam_ssi_rpi_ib_reqlist_t *rl, 
                               struct lam_ssi_rpi_ib_reqlist_item_t *rli)
{
  struct lam_ssi_rpi_ib_reqlist_item_t *next, *prev;
 ...
 */
int
lam_ssi_rpi_ib_reqlist_rm_req(struct lam_ssi_rpi_ib_reqlist_t *rl, 
			      MPI_Request req)
{
  int found = 0;
  struct lam_ssi_rpi_ib_reqlist_item_t *rli, *rli_temp;

  if (req != NULL) {
    for (rli = rl->top; rli != NULL; rli = rli_temp) {
      rli_temp = rli->next;
      if (rli->req == req) {
	if (lam_ssi_rpi_ib_reqlist_rm_item(rl, rli) == LAMERROR)
	  return LAMERROR;
 ...
 */
int
lam_ssi_rpi_ib_reqlist_pop_top(struct lam_ssi_rpi_ib_reqlist_t *rl)
{
  struct lam_ssi_rpi_ib_reqlist_item_t *rli;
