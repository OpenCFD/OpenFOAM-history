 ...
 * $HEADER$
 *
 * $Id: ssi_rpi_usysv.c,v 1.34 2003/10/11 19:15:38 brbarret Exp $
 *
 ...
 * local variables
 */
static int param_lock_poll_read;
static int param_lock_poll_write;
...
static const lam_ssi_rpi_actions_1_0_0_t module_actions = {
  ...
  lam_ssi_rpi_usysv_addprocs,
  lam_ssi_rpi_usysv_finalize,

  lam_ssi_rpi_usysv_build,
  lam_ssi_rpi_usysv_start,
  lam_ssi_rpi_usysv_advance,
  lam_ssi_rpi_usysv_destroy,

  lam_ssi_rpi_usysv_iprobe,

  lam_ssi_rpi_usysv_fastrecv,
  lam_ssi_rpi_usysv_fastsend,
 ...
 */
int
lam_ssi_rpi_usysv_open_module(OPT *ad)
{
  param_lock_poll_read =
    lam_ssi_base_param_register_int("rpi", "usysv", "readlockpoll", NULL, 
                                    READPOLLCOUNT);
  param_lock_poll_write =
    lam_ssi_base_param_register_int("rpi", "usysv", "writelockpoll", NULL, 
                                    WRITEPOLLCOUNT);

  param_poll_yield =
    lam_ssi_base_param_register_int("rpi", "usysv", "pollyield", NULL, 1);
  param_poolsize =
    lam_ssi_base_param_register_int("rpi", "usysv", "poolsize", NULL,
                                    LAM_SHMPOOLSIZE);
  param_maxalloc =
    lam_ssi_base_param_register_int("rpi", "usysv", "maxalloc", NULL,
                                    LAM_SHMMAXALLOC);
  param_short = 
    lam_ssi_base_param_register_int("rpi", "usysv", "short", NULL,
                                    LAM_SHMSHORTMSGLEN);
  ...
  param_priority = 
    lam_ssi_base_param_register_int("rpi", "usysv", "priority", NULL,
                                    LAM_SSI_RPI_USYSV_DEFAULT ? 75 : 40);
 ...
 */
int
lam_ssi_rpi_usysv_query(int *priority, int *thread_min, int *thread_max)
{
  ...
  *thread_max = MPI_THREAD_SERIALIZED;
  ...
  if (lam_ssi_rpi_verbose >= 5) {
    lam_debug(lam_ssi_rpi_did, "usysv: module initializing");
    lam_debug(lam_ssi_rpi_did, "usysv:verbose: %d", 
	      lam_ssi_rpi_verbose);
    lam_debug(lam_ssi_rpi_did, "usysv:priority: %d", *priority);
  }
		       ...
		       int *maxtag, int *maxcid)
{
  ...
  lam_ssi_rpi_usysv_lock_poll_read =
    lam_ssi_base_param_lookup_int(param_lock_poll_read);
  lam_ssi_rpi_usysv_lock_poll_write =
    lam_ssi_base_param_lookup_int(param_lock_poll_write);

  lam_ssi_rpi_usysv_poll_yield =
    lam_ssi_base_param_lookup_int(param_poll_yield);
  lam_ssi_rpi_usysv_short =
    lam_ssi_base_param_lookup_int(param_short);
  lam_ssi_rpi_usysv_poolsize =
    lam_ssi_base_param_lookup_int(param_poolsize);
  lam_ssi_rpi_usysv_maxalloc =
  ...
  lam_ssi_rpi_usysv_maxalloc = lam_ssi_rpi_usysv_maxalloc >> LAM_LOG_ALIGN;

  if (lam_ssi_rpi_verbose >= 0) {
    lam_debug(lam_ssi_rpi_did, "usysv: module initializing");
    lam_debug(lam_ssi_rpi_did, "usysv:pollyield: %d", 
	      lam_ssi_rpi_usysv_poll_yield);
    lam_debug(lam_ssi_rpi_did, "usysv:short: %d bytes", 
	      lam_ssi_rpi_usysv_short);
    lam_debug(lam_ssi_rpi_did, "usysv:shmpoolsize: %d bytes", 
	      lam_ssi_rpi_usysv_poolsize);
    lam_debug(lam_ssi_rpi_did, "usysv:shmmaxalloc: %d bytes", 
	      lam_ssi_rpi_usysv_maxalloc);
    lam_debug(lam_ssi_rpi_did, "usysv:readlockpoll: %d cycles", 
	      lam_ssi_rpi_usysv_lock_poll_read);
    lam_debug(lam_ssi_rpi_did, "usysv:writelockpoll: %d cycles", 
	      lam_ssi_rpi_usysv_lock_poll_write);
  ...
  if (lam_ssi_rpi_usysv_addprocs(procs, nprocs))
    return NULL;
 ...
 */
int
lam_ssi_rpi_usysv_addprocs(struct _proc **procs, int nprocs)
{
  int i, added = 0;
 ...
 */
int
lam_ssi_rpi_usysv_build(MPI_Request req)
{
 ...
 *			  lam_ssi_rpi_usysv_build() to optimize persistent requests
 *	Accepts:	- request list
 *			- request
 *	Returns:	- 0 or LAMERROR
 */
int
lam_ssi_rpi_usysv_start(MPI_Request req_top, MPI_Request req)
{
 ...
 */
    if ((bmsg = lam_ssi_rpi_cbuf_find(&req->rq_rpi->cq_env))) {
      lam_ssi_rpi_usysv_fill_wildcards(req, &bmsg->cm_env);
      if (req->rq_proc->p_rpi->cp_inbox != 0)
	return (lam_ssi_rpi_usysv_buffered_adv(req, bmsg));
      else
 ...
 */
    req->rq_rpi->cq_adv = (req->rq_type == LAM_RQIRECV) ?
      lam_ssi_rpi_usysv_shmtcp_req_recv : lam_ssi_rpi_usysv_shmtcp_req_probe;
  }
/*
 ...
 * using shared memory
 */
	if (req->rq_packsize > lam_ssi_rpi_usysv_short) {
/*
 * long message protocol
 */
	  req->rq_rpi->cq_nmsgout = lam_ssi_rpi_usysv_short;
	  req->rq_rpi->cq_nenvout = sizeof(struct lam_ssi_rpi_envl);
	  req->rq_rpi->cq_env.ce_flags |= C2CLONG;
	  req->rq_rpi->cq_adv = lam_ssi_rpi_usysv_req_send_long;
	} else {
/*
 ...
 */
	  ...
	  if (req->rq_type == LAM_RQISSEND) {
	    ...
	    req->rq_rpi->cq_adv = lam_ssi_rpi_usysv_req_send_synch;
	  } else
	    req->rq_rpi->cq_adv = lam_ssi_rpi_usysv_req_send_short;
	}
      } else {
/*
 * using TCP
 ...
 */
int
lam_ssi_rpi_usysv_destroy(MPI_Request req)
{
  if (req->rq_rpi != 0)
    free(req->rq_rpi);
  req->rq_rpi = 0;
 ...
 */
int
lam_ssi_rpi_usysv_advance(MPI_Request req_top, int fl_block)
{
 ...
 */
  ...
  if (lam_ger && lam_ssi_rpi_tcp_flblock
      && (lam_ssi_rpi_usysv_nread + lam_ssi_rpi_usysv_nwrite + 
	  lam_ssi_rpi_tcp_nio == 0)) {
    errno = EGERFLOW;
    return (LAMERROR);
  }

  return (lam_ssi_rpi_usysv_advance_common());
}
 ...
 */
int
lam_ssi_rpi_usysv_iprobe(MPI_Request req)
{
  int err;			/* error code */
/*
 ...
 */
int
lam_ssi_rpi_usysv_finalize(struct _proc *p)
{
  if (p)
    return (finalize1(p));
  else {
 ...
 */
    if (lam_ssi_rpi_usysv_membase)
      if (lam_ssi_rpi_usysv_cleanup(lam_myproc->p_rpi))
	return LAMERROR;
/*
 ...
 */
    ...
    free(lam_ssi_rpi_usysv_read);
    free(lam_ssi_rpi_usysv_write);

    if (lam_ssi_rpi_verbose >= 0)
      lam_debug(lam_ssi_rpi_did, "usysv: module finalizing");
  }
 ...
 */
  for (recv = req_top; recv; recv = recv->rq_next)
	...
	(!lam_ssi_rpi_envl_cmp(&send->rq_rpi->cq_env, &recv->rq_rpi->cq_env)))
 ...
 */
    ...
    ps->cp_write = 1;
    lam_ssi_rpi_usysv_write[lam_ssi_rpi_usysv_nwrite++] = ps;
  } else {
/*
 * using TCP
 */
 ...
 */
    ...
    ps->cp_read = 1;
    ps->cp_mreq = req;
    lam_ssi_rpi_usysv_read[lam_ssi_rpi_usysv_nread++] = ps;
 ...
 */
  if (lam_comms == 0) {
    ...
    if (nshm > 1 && lowp == lam_myproc) {
      ...
      if (lam_ssi_rpi_usysv_create_area(nshm, lam_myproc->p_rpi, &outmsg))
	return (LAMERROR);
    }
  }
/*
 ...
 */
	...
	rport = (int) inmsg.nh_data[0];
        ...
        if (lam_ssi_rpi_verbose >= 5) {
          lam_debug(lam_ssi_rpi_did, "usysv: creating outgoing socket to %s",
 ...
 */
  if (lam_comms == 0 && nshm > 1) {
	...
	return (LAMERROR);
      ...
      if (lam_ssi_rpi_usysv_attach_area(nshm, lam_myproc->p_rpi, &inmsg))
	return (LAMERROR);
    }
/*
 ...
 */
	...
	continue;
      ...
      p->p_rpi->cp_inbox = (lam_ssi_rpi_usysv_postbox_t *) 
	(lam_ssi_rpi_usysv_membase + CACHELINESIZE + SHMBOXSIZE *
	 (lam_myproc->p_rpi->cp_shmidx * nshm + p->p_rpi->cp_shmidx));
      p->p_rpi->cp_outbox = (lam_ssi_rpi_usysv_postbox_t *) 
	(lam_ssi_rpi_usysv_membase + CACHELINESIZE + SHMBOXSIZE * 
 ...
 */
  if (lam_ssi_rpi_usysv_nprocs > lam_ssi_rpi_usysv_narray) {
    if (lam_ssi_rpi_usysv_narray == 0) {
      lam_ssi_rpi_usysv_read = (struct lam_ssi_rpi_proc **)
	malloc(lam_ssi_rpi_usysv_nprocs * sizeof(struct lam_ssi_rpi_proc *));
      lam_ssi_rpi_usysv_write = (struct lam_ssi_rpi_proc **)
	malloc(lam_ssi_rpi_usysv_nprocs * sizeof(struct lam_ssi_rpi_proc *));
    } else {
      lam_ssi_rpi_usysv_read = (struct lam_ssi_rpi_proc **)
	realloc(lam_ssi_rpi_usysv_read, lam_ssi_rpi_usysv_nprocs *
		sizeof(struct lam_ssi_rpi_proc *));
      lam_ssi_rpi_usysv_write = (struct lam_ssi_rpi_proc **)
	realloc(lam_ssi_rpi_usysv_write, lam_ssi_rpi_usysv_nprocs *
		sizeof(struct lam_ssi_rpi_proc *));
    }

    if (lam_ssi_rpi_usysv_read == 0 || lam_ssi_rpi_usysv_write == 0)
      return (LAMERROR);
    lam_ssi_rpi_usysv_narray = lam_ssi_rpi_usysv_nprocs;
  }
...
proc_init(struct _proc *p)
{
  ...
  if (!(p->p_mode & LAM_PCLIENT)
    ...
    p->p_rpi->cp_readfn = lam_ssi_rpi_usysv_proc_read_env;
  } else {
 ...
 */
int
lam_ssi_rpi_usysv_fastsend(char *buf, int count, MPI_Datatype dtype,
 ...
 */
  if (destproc->p_rpi->cp_inbox != 0)
    err = lam_ssi_rpi_usysv_low_fastsend(packbuf, packsize,
 ...
 */
int
lam_ssi_rpi_usysv_fastrecv(char *buf, int count, MPI_Datatype dtype,
 ...
 */
  if (srcproc->p_rpi->cp_sock < 0)
    err = lam_ssi_rpi_usysv_low_fastrecv(packbuf, &packsize,
					 srcproc->p_rpi, src, tag, 
					 comm, seqnum);
  else
