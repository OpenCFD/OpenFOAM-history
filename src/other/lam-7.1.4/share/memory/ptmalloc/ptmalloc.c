...
/* $Id: ptmalloc.c,v 1.4 2003/09/19 20:13:18 jsquyres Exp $
 ...
 * Hook functions
 */
extern void *lam_ptmalloc_sbrk(int inc);
extern int lam_ptmalloc_munmap(void *start, size_t length);
...
#define MORECORE lam_ptmalloc_sbrk
#define munmap(a, b) lam_ptmalloc_munmap(a, b)
...
#define malloc_getpagesize __libc_pagesize
  ...
  /* LAM: Only declare this if it's not already declared, because some
     compilers (cough cough Portland) define ptrdiff_t to be
     "something different" than expected (i.e., something different
     than what is defined by the back-end of <stddef.h>), and
     therefore creates a mismatched prototype, and causes a compiler
     error.  So -- if it's already prototyped by the compiler/OS
     header files, leave it be. */
#if __STD_C
...
#endif
{
  ...
  /* LAM/MPI: There are obscure places where the memory will *not*
     have been allocated by this module.  Concrete example: C code
     that is compiled into a matlab MEX executable, and during
     MPI_INIT we call strdup() followed by a free().  The strdup()
     apparently calls some other malloc(), but the explicit free()
     will invoke this function -- but since the memory was not
     allocated here, ar_ptr will be NULL.  So add an if() block around
     the processing code. */
