 ...
 * BEGIN LAM CHANGES
 *
 * - Increase the default mmap threshold so that we don't give back memory
 *   all the time (unpinning is expensive)
 * - Make sure we use MREMAP, since it doesn't seem to build
 * - Make sure our hooks get called
 * - added checks for case where memset is a macro so that we don't
 *   cause compile badness otherwise
 */
#include <lam_config.h>
#include <malloc_wrapper.h>

extern void *lam_ptmalloc2_sbrk(int inc);
extern int lam_ptmalloc2_munmap(void *start, size_t length);

#define MORECORE lam_ptmalloc2_sbrk
#define munmap(a,b) lam_ptmalloc2_munmap(a,b)
...
#endif
/* CONST is a #define in lam_config.h, basically to const */
...
#else
#if __STD_C
#ifndef memset /* LAM CHANGE */
void* memset(void*, int, size_t);
#endif
#ifndef memcpy /* LAM CHANGE */
...
public_mALLOc(size_t bytes)
{
  ...
  if(!victim) {
    ...
    if(ar_ptr != &main_arena) {
      ...
      ar_ptr = arena_get2(ar_ptr->next ? ar_ptr : NULL, bytes);
      (void)mutex_unlock(&main_arena.mutex);
      if(ar_ptr) {
        victim = _int_malloc(ar_ptr, bytes);
        (void)mutex_unlock(&ar_ptr->mutex);
...
public_cALLOc(size_t n, size_t elem_size)
{
  ...
  if (mem == 0) {
    ...
    if(av != &main_arena) {
      ...
      /* ... or sbrk() has failed and there is still a chance to mmap() */
      (void)mutex_lock(&main_arena.mutex);
      av = arena_get2(av->next ? av : NULL, sz);
      (void)mutex_unlock(&main_arena.mutex);
      if(av) {
        mem = _int_malloc(av, sz);
        (void)mutex_unlock(&av->mutex);
