 ...
 * $HEADER$
 *
 * $Id: p_main.c,v 6.15 2003/04/09 03:37:46 brbarret Exp $
 * 
 *	Function:	- remote process server
 */
...
main(int argc, char *argv[])
{
  ...
  if (ao_taken(lam_daemon_optd, "d")) {
	...
	_lam_signal(SIGHUP, pbail);
/*
 * Attach to kernel.
 */
	if (kinit(PRDAEMON)) lampanic("kenyad (kinit)");
/*
 * Initialize.
 */
	p_init(&argc, argv);
 ...
 *	pbail
 *
 *	Function:	- clean up before exiting
 */
static void
pbail()
