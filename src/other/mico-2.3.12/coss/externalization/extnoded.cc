...
usage(const char* progName)
{
	...
	"-v, --version             Show CosCompoundExternalization::Node Service Factories version.\n"
	"-n, --with-naming         Try to register CosCompoundExternalization::Node Service with Naming Service.\n"
	"-t, --with-trading        Try to register CosCompoundExternalization::Node Service with Trading Service.\n"
	;
}
...
int main( int argc, char *argv[] )
{
    ...
    NodeStreamableFactory_impl<CosCompoundExternalization_impl::Node_impl>* node_factory =
	    new NodeStreamableFactory_impl<CosCompoundExternalization_impl::Node_impl> (orb);

    PortableServer::ObjectId_var node_factory_id =
	    PortableServer::string_to_ObjectId ("CosCompoundExternalization_Node_impl");
	...
	switch (type)
	{
		...
		case RegisterHelper::trading:
		{
			...
			CosNaming::Name node_id(4);
			node_id.length(4);
			node_id[0].kind = CORBA::string_dup( _lc_ks_object_interface );
			node_id[0].id = CORBA::string_dup( "::CosCompoundExternalization::Node" );
			node_id[1].kind = CORBA::string_dup( _lc_ks_impl_equiv_class );
			node_id[1].id = CORBA::string_dup("::CosStream::Streamable");
			...
			node_id[3].id = CORBA::string_dup( "NodeStreamableFactory" );
