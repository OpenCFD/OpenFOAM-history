...
//  ObjectDomainMapping::Factory_var factory = odm->create(); // creates new ODM

//  CORBA::Boolean res = factory->loadConfigFile(NULL); // use filename from arguments
  // this ODM now contains default ODM and other entries specified in configuration file
//  poa->registerODMFactory(factory);

  // create a domain managers tree, grant rights
...
//  CORBA::Boolean sf = dmfactory->loadConfigFile("access.cnf");
  
  // Set up required rights
...
//  sf = rr->loadConfigFile("rights.cnf");
